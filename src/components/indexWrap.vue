<template>
  <div class="title_div">
	  <van-row type="flex" justify="space-between" style='padding:0 .2rem;'>
	    ...
	    <van-col @click='searchBtnFn' class='search_div' span="16">
			<van-icon name="search" class='search_icon' size="40" />
			<span>请输入商品名称…</span>
		</van-col>
	    <van-col span="3">
	  ...
	  <van-tabs 
		  ...
		  swipeable 
		  class='tabBtnWrap'>
	    <van-tab v-for="(n,inx) in tabBtnArr" :key=inx :title="n">
	      <!-- {{ n }} -->
	    </van-tab>
	  </van-tabs>
	  ...
	  <div class="indexServicePolicy">
		  <ul>
			  <li>
			    <i style='background-image:url(http://yanxuan.nosdn.127.net/a03dd909803b9ac032eba58b7253a2f6.png);'></i>网易自营品牌
			  </li>
			  <li>
			    <i style='background-image:url(http://yanxuan.nosdn.127.net/2d0402ffcd52b3ec3b07422681c42a89.png);'></i>30天无忧退货
			  </li>
			  <li>
			    <i style='background-image:url(http://yanxuan.nosdn.127.net/eb61ee48e8942dbd1784c9ee75ebe955.png);'></i>48小时快速退款
			  </li>
		  </ul>
	  </div>
	  
	  <!-- 宫格部分 -->
	  <!-- 报很多错误，应该是key错误，改成（n,inx） -->
	  <div class="goodsColumn_wrap"> 
		  <van-grid :column-num="5">
		    <van-grid-item 
				v-for="(n,inx) in goodsColum_arr" 
				:key=inx 
				:icon=n.img_url
				:text=n.img_txt
			>
...
export default {
  ...
  created(){
	  axios.get('http://localhost:3333/get_tabBtn_list')
		.then(_d =>{
			// console.log(_d.data)
			this.tabBtnArr = _d.data;
		})
	
	axios.get('http://localhost:3333/get_swip_img')
			.then(_d =>{
				// console.log(_d.data)
